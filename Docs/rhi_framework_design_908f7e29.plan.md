---
name: RHI Framework Design
overview: 设计一套支持DX11/DX12/Vulkan的跨平台RHI抽象层，采用多线程命令录制、引用计数资源管理、运行时Shader编译和RenderGraph自动屏障管理。
todos:
  - id: p1-setup
    content: P1.1 创建目录结构、vcpkg.json、CMakeLists.txt
    status: completed
  - id: p1-core
    content: P1.2 实现Core模块：RefCounted、IDeferredDeleter、Handle
    status: completed
  - id: p2-definitions
    content: P2.1 定义RHI枚举、Format、Subresource、Capabilities
    status: completed
  - id: p2-resources
    content: P2.2 实现RHI资源接口：Buffer、Texture、Sampler、Shader
    status: completed
  - id: p2-pipeline
    content: P2.3 实现Pipeline接口：PSO、RenderPass、InputLayout
    status: completed
  - id: p2-descriptor
    content: P2.4 实现描述符系统：BindGroup、Layout、Allocator
    status: completed
  - id: p2-command
    content: P2.5 实现CommandContext接口：绑定、绘制、屏障、调试标记
    status: completed
  - id: p2-device
    content: P2.6 实现Device接口：资源创建、队列提交、帧管理
    status: completed
  - id: p2-shader-compiler
    content: P2.7 集成DXC：HLSL编译、反射、变体管理
    status: pending
  - id: p3-dx12-device
    content: P3.1 DX12 Device：适配器枚举、队列创建、SwapChain
    status: pending
  - id: p3-dx12-resources
    content: P3.2 DX12 Resources：CommittedResource、D3D12MA集成
    status: pending
  - id: p3-dx12-descriptor
    content: P3.3 DX12 Descriptor：DescriptorHeap、RootSignature缓存
    status: pending
  - id: p3-dx12-command
    content: P3.4 DX12 Command：CommandList、Barrier、ExecuteIndirect
    status: pending
  - id: p3-dx12-pipeline
    content: P3.5 DX12 Pipeline：PSO创建、PipelineCache
    status: pending
  - id: p3-dx12-validation
    content: P3.6 DX12 验证：Triangle渲染、资源生命周期、多帧测试
    status: pending
  - id: p4-vk-device
    content: P4.1 Vulkan Device：Instance、PhysicalDevice、LogicalDevice
    status: pending
  - id: p4-vk-resources
    content: P4.2 Vulkan Resources：VMA集成、Image、Buffer
    status: pending
  - id: p4-vk-descriptor
    content: P4.3 Vulkan Descriptor：DescriptorPool、SetLayout
    status: pending
  - id: p4-vk-command
    content: P4.4 Vulkan Command：CommandBuffer、RenderPass、Barrier
    status: pending
  - id: p4-vk-pipeline
    content: P4.5 Vulkan Pipeline：GraphicsPipeline、ComputePipeline
    status: pending
  - id: p4-vk-validation
    content: P4.6 Vulkan 验证：Triangle渲染、Validation Layer、GPU调试
    status: pending
  - id: p5-dx11-device
    content: P5.1 DX11 Device：Device创建、Context、SwapChain
    status: pending
  - id: p5-dx11-resources
    content: P5.2 DX11 Resources：Buffer、Texture、SRV/UAV/RTV
    status: pending
  - id: p5-dx11-binding
    content: P5.3 DX11 Binding：Slot映射、扁平化绑定、Remapper
    status: pending
  - id: p5-dx11-command
    content: P5.4 DX11 Command：ImmediateContext、DeferredContext适配
    status: pending
  - id: p5-dx11-pipeline
    content: P5.5 DX11 Pipeline：InputLayout、BlendState、RasterState
    status: pending
  - id: p5-dx11-validation
    content: P5.6 DX11 验证：Triangle渲染、多线程降级、Debug Layer
    status: pending
  - id: p6-rendergraph
    content: P6.1 RenderGraph：DAG编译、Subresource屏障、资源别名
    status: pending
  - id: p6-parallel
    content: P6.2 并行执行：多线程录制、任务分发
    status: pending
  - id: p6-validation
    content: P6.3 RenderGraph 验证：复杂Pass依赖、性能测试
    status: pending
  - id: p7-cross-backend
    content: P7.1 跨后端验证：同一场景在三个后端渲染一致性
    status: pending
  - id: p7-stress-test
    content: P7.2 压力测试：大量DrawCall、资源创建销毁
    status: pending
---
