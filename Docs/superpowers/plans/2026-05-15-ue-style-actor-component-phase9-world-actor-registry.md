...
- [ ] Add a helper `WorldManagedPureActor : public RVX::Actor` with a constructor that creates a root `SceneComponent`:
...
- [ ] Add a helper component that records registration, initialization, begin, tick, end, unregister, and destroy counts:
...
- [ ] Add a helper component that requests world destruction from inside `TickComponent()`:
...
- [ ] Add `Test_WorldSpawnPureActorOwnsAndLooksUpActor`:
  - Initialize `World`.
...
- [ ] Add `Test_WorldSpawnSceneEntityStillDelegatesToSceneManager`:
...
- [ ] Add `Test_WorldPureActorLifecycleBeginsTicksAndDestroys`:
...
- [ ] Add `Test_WorldDestroyPureActorDefersDuringLifecycleDispatch`:
  - Spawn `WorldManagedPureActor`.
...
- [ ] Add `Test_WorldDestroyPureActorRejectsDuplicatePendingDestroy`:
  - Spawn `WorldManagedPureActor`.
...
- [ ] Add `Test_WorldPureActorRejectsSceneParent`:
...
- [ ] Add `Test_WorldUnloadClearsPureActorsAndSceneActors`:
...
- [ ] Run `ActorComponentValidation` and confirm it fails to compile before implementation because `World::GetActor`, `GetActorCount`, and pure `SpawnActor<T>()` are not implemented.
...
- [ ] Update `Actor::ShouldAutoRegisterComponent()`:
...
- [ ] Preserve `SceneEntity::ShouldAutoRegisterComponent()` unchanged. Scene-managed entities should continue using `m_sceneManager != nullptr`.
...
- [ ] Add public actor registry APIs to `World`:
...
- [ ] Replace the existing `World::SpawnActor<T>()` template with an Actor-based dispatch:
...
- [ ] Add private registry fields to `World`:
...
- [ ] Update non-template `World::SpawnActor(const ActorSpawnParams&)` to continue spawning a `SceneEntity`.

- [ ] Implement `World::GetActor()`:
...
- [ ] Replace `World::DestroyActor(Actor*)`:
...
- [ ] Implement `UpdatePureActorLifecycles(float deltaTime)`:
...
- [ ] Update `World::Tick(float deltaTime)`:
...
- [ ] Implement `DestroyPureActorImmediate()`:
  - Find actor in `m_actors`.
  - Call `EndPlay()`.
  - Call `UnregisterAllComponents()`.
  - Erase from `m_actors`, letting the destructor call `OnComponentDestroyed()`.

- [ ] Implement `ClearPureActors()`:
  - Clear dispatch state and pending list.
  - For every actor: `EndPlay()` then `UnregisterAllComponents()`.
  - Clear `m_actors`.

- [ ] Call `ClearPureActors()` in `World::Unload()` before resetting the scene manager.

- [ ] Call `ClearPureActors()` in `World::Shutdown()` before subsystem deinitialization and scene manager shutdown.
...
- [ ] Run one GPT-5.3-Codex-Spark code review after validation. Fix any Critical/Important findings before committing.
