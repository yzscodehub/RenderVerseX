...
**日期**：2026-06-26  
**状态**：北极星能力分级 · v1.1 目标口径校准

本文定义 RVX-NG 的能力分级规则：首期主线必须能稳定出货，高端能力可选，研究级能力不得阻塞主线。v1.1 目标口径下，能力分级服务于“生产级框架底座 + 分级演进上限”：首期只承诺可编译、可测试、可诊断、可扩展、可工具化的 Proven 框架能力；Advanced/Research 必须以开关、fallback、诊断和测试证明不会拖垮主线。每个能力的测试门禁映射见 `capability-test-matrix.md`。
...
- 任何能力没有真实运行路径、明确 fallback、诊断 artifact 和自动化门禁前，不得计入首期框架完成。
