...
**日期**：2026-06-20
**状态**：框架 v1.1 目标口径调整（v1.0 冻结项不变 · ambition 分级兑现）；Spike 与实施方案后置
**性质**：RenderVerseX **次世代目标引擎架构**——目标不是一次性追逐所有 AAA 终态特性，而是先建立可编译、可测试、可诊断、可扩展、可工具化的生产级 C++20 实时渲染/游戏引擎框架，再按 Proven / Advanced / Research 分级演进。机制层对齐 UE5/Frostbite 级务实 SOTA；高风险能力（VisBuffer/ReSTIR/分层材质/Work Graphs）均**分级为"可叠加档/Phase-2"**，首期主线全部跑已验证路线（见 §25）。本文只设计**框架**（分层、模块职责、边界、数据流、决策、里程碑、工程化骨架），**不含代码**。各子系统细化另起子 spec。
...
- **整体目标**：把 RenderVerseX 建设成一个以现代 RenderGraph、多后端 RHI 和清晰数据边界为核心的生产级 C++20 实时渲染/游戏引擎框架。它首先要成为稳定框架底座，而不是一次性承诺所有终态功能。
- **近期目标**：先交付可靠的引擎骨架：清晰模块分层、诚实 capability、RenderGraph 中心化 GPU 工作表达、Scene/World 到 Render 的快照边界、资源/Shader/PSO/cook 的可测试闭环、诊断 artifact，以及能在 CI 中持续运行的架构门禁。
- **中期目标**：在骨架稳定后补齐生产工作流：Editor 与 runtime 同源、资源 cook/DDC/VFS、GpuScene、GPU-driven 渲染、FrameDebugger/Profiler、质量档、跨后端回归和可观测诊断。
- **长期上限**：支持开放世界、大规模场景、高端 GI/RT、主机/移动/XR profile 和完整发行链路；这些按 Proven / Advanced / Research 分级演进，不是首期一次性交付范围。
...
首期目标按“框架是否可持续演进”验收，而不是按“是否已经覆盖全部终态功能”验收。一个能力进入首期主线，必须同时满足：

- 有清晰模块归属、公共接口、不变量和失败语义。
- 有真实运行路径或明确 fallback，不能用 stub 假装成功。
- 有 capability、quality tier 或 policy 描述它何时可用、何时降级、何时跳过。
- 有自动化测试、架构门禁、诊断 artifact 或可复用 smoke gate。
- 有 Editor/runtime/CI/tooling 可以消费的状态输出，避免只能人工看样例。

不满足这些条件的能力只能停留在 Advanced/Research、Spike 或后续实施方案中，不能作为首期完成条件。
