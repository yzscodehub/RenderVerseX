---
name: 游戏引擎最佳实践重构
overview: 基于 Unreal Engine、Unity 等主流引擎的最佳实践，采用 Subsystem 模式、Event Bus、模块化边界、数据驱动等设计原则，将 RenderVerseX 重构为专业级游戏引擎架构。
todos:
  - id: phase1-core
    content: "Phase 1: Core 基础设施 - EventBus + Subsystem + JobSystem"
    status: completed
  - id: phase2-hal
    content: "Phase 2: 跳过 - HAL 迁移改为保留 Platform/Input"
    status: completed
  - id: phase3-engine
    content: "Phase 3: 重构 Engine 核心 - SubsystemCollection + ModuleManager"
    status: completed
  - id: phase4-world
    content: "Phase 4: 创建 World 模块 - 整合 Scene/Spatial/Picking"
    status: pending
  - id: phase5-runtime
    content: "Phase 5: 创建 Runtime 模块 - Window/Input/Time 已创建, Camera 待迁移"
    status: in_progress
  - id: phase6-render
    content: "Phase 6: 重构 Render 模块 - RenderSubsystem + 合并 RenderGraph"
    status: pending
  - id: phase7-resource
    content: "Phase 7: 创建 Resource 模块 - ResourceSubsystem"
    status: pending
  - id: phase8-cleanup
    content: "Phase 8: 清理旧模块 + 更新 Tests/Samples"
    status: pending
---
